...
| **Writeup** | After completion, synthesizes the successful path into a reproduction writeup | Writeup markdown |
...
**Cairn Dispatcher** reads the graph, schedules tasks, manages per-project workspaces on the host, and is the sole writer to the protocol. Each project gets its own working directory; Agent Workers run as local subprocesses inside it, reusing the machine's already-configured and logged-in `claude` / `codex` / `pi` CLIs — no Docker, no API keys in the config. Agent Workers only receive a prompt and return structured output.
...
- macOS or Linux
- Python ≥ 3.12
- At least one of the `claude` / `codex` / `pi` CLIs installed, on `PATH`, and already logged in

### Setup

Workers run directly on the dispatcher host as local subprocesses, reusing the machine's already-configured `claude` / `codex` / `pi` CLIs — no Docker, and no API keys in the config.
